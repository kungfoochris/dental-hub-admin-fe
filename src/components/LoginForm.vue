  ...
  methods:{
    ...
    processLogin(){
      ...
      if(this.$data.username==''){
        ...
        axios.post('http://192.168.1.69:6061/api/v1/checkuser',{'email':this.$data.username}).then(response=>{
          this.setAdminUsername(response.data.username)
          var formData = {'username': this.adminusername, 'password': this.$data.password}
          axios.post('http://192.168.1.69:6061/api/v1/token/obtain',formData)
          .then(response => {
