  ...
  methods:{
    ...
    processLogin(){
      ...
      if(this.$data.username==''){
        ...
        axios.post('https://app.abhiyantrik.com/api/v1/checkuser',{'email':this.$data.username}).then(response=>{
          // this.setAdminUsername(response.data.username)
          var formData = {'username': response.data.username, 'password': this.$data.password}
          axios.post('https://app.abhiyantrik.com/api/v1/token',formData)
          .then(response => {
